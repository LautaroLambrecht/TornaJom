...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        $modelo->drawWorkId($_GET['id']);
        echo "
            <div style='display:flex; justify-content:center;flex-direction:row;align-items:center; gap:30px'> 
                <form action='updateEstado.php' method='get'>
                    <input type='hidden' name='id' value='".$_GET['id']."'>
                    <input type='hidden' name='id_realizacion' value='".$usuario_id."'>
                    <input type='hidden' name='estado' value='reclamado'>
                    <div>
                        <button style='width: auto;' type='submit'>Reclamar</button>
                    </div>
                </form>
